...
from power_fake_toys import generate_one
...
if __name__ == '__main__':
    ...
    opt_args.add_argument("--out-format", type=str, help="output format, csv or json, default csv",
                          choices=["csv", "json"],
                          default="csv")
    opt_args.add_argument("--key", type=str, help="PINBlock Key, 32H for SM4 and 3DES PINBlock, if no will no Cipher",
                          default=None)
    ...
    if args.out_file:
        writer = open(args.out_file + '_%s.%s' % (args.key.upper() if key is not None else 'PLAIN', args.out_format),
                      mode="w")
    else:
        writer = sys.stdout
    ...
    if args.out_format == 'csv':
        if args.key is not None:
            csv_header = ['PAN', 'PIN', 'PinAccount', 'PlainPINBlock_GJ', 'PlainPINBlock_GM',
                          'CipherPINBlock_GJ', 'CipherPINBlock_GM']
