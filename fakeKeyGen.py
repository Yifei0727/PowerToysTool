...
from power_fake_toys import generate_key_by_kek
...
if __name__ == '__main__':
    ...
    opt_args.add_argument("--key", type=str, required=True,
                          help="32H for SM4 and 3DES  ,The Master Key (ZMK), eg. 11111111111111111111111111111111")
    opt_args.add_argument("-n", "--num-records", type=int, help="generate how many records as output, default 1",
                          default=1)
    opt_args.add_argument("--alg", required=True, choices=("sm4", "des"), type=str,
                          help="the algorithm how `key` encrypt work key ")
    ...
    if args.out_format == 'csv':
        csv_header = ['WK', 'ALG', 'CHK', 'CHK']
        csv_writer = csv.writer(writer)
        csv_writer.writerow(csv_header)

    for i in range(args.num_records):
        one_rec = generate_key_by_kek(key, args.alg)
        if args.out_format == 'csv':
